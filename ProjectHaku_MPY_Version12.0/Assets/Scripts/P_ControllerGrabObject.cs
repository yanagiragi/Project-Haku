...
public class P_ControllerGrabObject : MonoBehaviour
{
    ...
    private void GrabObject()
    {
        ...
        {
            // Debug.LogError("Get Sword with no SwordReturnTrigger: " + collidingObject); 
        }
    ...
    private void ReleaseObject()
    {
        ...
        if (objectInHand.CompareTag("C8763"))
        {
            ...
            {
                // 找另一把還在手上的劍，撥放音效
                if (a.gameObject.name != objectInHand.name && a.gameObject.GetComponent<Y_SwordReturnTrigger>().isGrab)
                {
                    if(!a.isPlaying) // 避免中斷已經正在撥放的音樂
                        a.Play();
                    break;
